...
const newsBrandAssets = [
  ...
  [/^canva-/, 'https://www.canva.com/favicon.ico', 'Canva logo'],
...
const latest = [
  ...newsItems.map(item => ({ title: item.title, href: `/news/${item.slug}/`, type: 'NEWS', icon: '&#128240;', className: 'is-news', date: item.datePublished })),
  ...posts.map(post => ({ title: post.title, href: `/blog/${post.slug}/`, type: 'NOTE', icon: '&#128221;', className: 'is-note', date: '' }))
].slice(0, 10);
const tickerLinks = [...latest, ...latest].map((item, index) => {
  ...
  return `                    <a class="guide-ticker-link ${item.className}" href="${item.href}"${hidden}><span class="ticker-type">${item.type}</span><span class="ticker-icon" aria-hidden="true">${item.icon}</span>${date}<span class="ticker-title">${esc(item.title)}</span></a>`;
}).join('\n');
