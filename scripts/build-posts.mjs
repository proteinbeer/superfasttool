...
function renderPost(post) {
  ...
  const schema = {
    ...
    datePublished: today,
    dateModified: today,
    author: { '@type': 'Organization', name: 'Super Fast Tool' },
...
<meta property="og:type" content="article"><meta property="og:title" content="${esc(post.title)}"><meta property="og:description" content="${esc(post.description)}"><meta property="og:url" content="${canonical}">
...
function renderNews(item) {
  ...
  return renderPost({
    ...
    .replace(`${esc(item.description)} This short article connects the topic to practical browser-based tools and everyday workflows.</p>`, `${esc(item.description)} This short news note connects the update to lightweight browser-based utility workflows.</p>`)
    .replace(`${toolLink}\n\n<div class="note">`, `${toolLink}\n${sections}\n<section><h2>Further reading</h2><ul>${sources}</ul></section>\n<div class="note">`);
}
